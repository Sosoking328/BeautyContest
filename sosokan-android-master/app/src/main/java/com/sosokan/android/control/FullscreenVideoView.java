package com.sosokan.android.control;
...
public class FullscreenVideoView extends RelativeLayout implements SurfaceHolder.Callback, MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnSeekCompleteListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnInfoListener, MediaPlayer.OnVideoSizeChangedListener {
    ...
    protected FullscreenVideoView.State currentState;
    protected FullscreenVideoView.State lastState; // Tells onSeekCompletion what to do
    ...
    protected void onDetachedFromWindow() {
        ...
        if (!detachedByFullscreen) {
            ...
            currentState = FullscreenVideoView.State.END;
        }
    ...
    synchronized public void surfaceCreated(SurfaceHolder holder) {
        ...
        if (!surfaceIsReady) {
            surfaceIsReady = true;
            if (currentState != FullscreenVideoView.State.PREPARED &&
                    currentState != FullscreenVideoView.State.PAUSED &&
                    currentState != FullscreenVideoView.State.STARTED &&
                    currentState != FullscreenVideoView.State.PLAYBACKCOMPLETED)
                tryToPrepare();
        }
    }
    ...
    public void onSeekComplete(MediaPlayer mp) {
        ...
        if (lastState != null) {
            switch (lastState) {
                ...
                case PREPARED: {
                    currentState = FullscreenVideoView.State.PREPARED;
                    break;
                }
            }
        }
        ...
        if (this.mediaPlayer != null) {
            if (this.currentState != FullscreenVideoView.State.ERROR) {
                ...
                if (!this.mediaPlayer.isLooping())
                    this.currentState = FullscreenVideoView.State.PLAYBACKCOMPLETED;
                else
                    start();
            }
        }
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        stopLoading();
        this.currentState = FullscreenVideoView.State.ERROR;
    ...
    protected void prepare() throws IllegalStateException {
        ...
        this.currentState = FullscreenVideoView.State.PREPARING;
     ...
     */
    synchronized public FullscreenVideoView.State getCurrentState() {
        return currentState;
    }
    ...
    public void stop() throws IllegalStateException {
        ...
        if (mediaPlayer != null) {
            currentState = FullscreenVideoView.State.STOPPED;
            mediaPlayer.stop();
        } else throw new RuntimeException("Media Player is not initialized");
    }
    ...
    public void setVideoPath(String path) throws IOException, IllegalStateException, SecurityException, IllegalArgumentException, RuntimeException {
        if (mediaPlayer != null) {
            if (currentState != FullscreenVideoView.State.IDLE)
                throw new IllegalStateException("FullscreenVideoView Invalid State: " + currentState);

            mediaPlayer.setDataSource(path);

            currentState = FullscreenVideoView.State.INITIALIZED;
            prepare();
        } else throw new RuntimeException("Media Player is not initialized");
    }
    ...
    public void setVideoURI(Uri uri) throws IOException, IllegalStateException, SecurityException, IllegalArgumentException, RuntimeException {
        if (mediaPlayer != null) {
            ...
            // throw new IllegalStateException("FullscreenVideoView Invalid State: " + currentState);
            {
                ...
                currentState = FullscreenVideoView.State.IDLE;
            }
