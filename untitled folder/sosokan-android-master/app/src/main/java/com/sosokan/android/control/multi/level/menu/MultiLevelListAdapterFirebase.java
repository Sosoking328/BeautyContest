 ...
 */
public abstract class MultiLevelListAdapterFirebase {

    private MultiLevelListViewFirebase mView;
    ...
    private List<Node> createNodeListFromDataItems(List<?> dataItems, Node parent) {
        ...
        Category category = (Category) parent.getObject();

        if(category!=null)
        dataItems = DataProviderCategory.getSubCategoryItems(category.getId());
   ...
   public List<Node> getFlatItems() {
     ...
     */
    void unregisterView(MultiLevelListViewFirebase view) {
        if (mView != view) {
     ...
     */
    void registerView(MultiLevelListViewFirebase view) {
